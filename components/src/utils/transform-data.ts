...
export function applyPivotByLabelTransform(
  ...
  let rowCol = nonEmpty(rowColumnName, rowFieldName);

  const resolveKey = (row: Record<string, unknown>, base: string): string | undefined => {
    if (hasOwn(row, base)) return base;
  ...
  for (const row of data) {
    const rowKeyName = resolveKey(row, rowFieldName);
    const labelKeyName = resolveKey(row, columnLabel);
    const valueKeyName = resolveKey(row, valueFieldName);
