...
export function applyCustomLabel(formatted: string, customLabel?: string, unit?: string): string {
  ...
  // Time units: Intl narrow often glues the suffix ("1ms", "1μs") with no space — the
  // generic leadingQuantity path can leave "ms" if a consumer re-appends a label, and
  // the bare append fallback would yield "1ms prout". Strip known time tokens first.
  if (unit && TIME_FORMAT_UNITS.has(unit)) {
    return applyTimeCustomLabel(formatted, label);
  }

  // Decimal / throughput fallback: keep leading quantity only
...
/** Time format unit keys (formatTime / Intl unit style). */
const TIME_FORMAT_UNITS = new Set([
  'nanoseconds',
  'microseconds',
  'milliseconds',
  'seconds',
...
/**
 * Replace formatTime output with custom label.
 * Handles glued narrow units (1ms, 500µs), spaced long forms (1 month), and
 * multi-part strings (1d255ms) by keeping only the leading quantity (+ compact K/M/B/T).
 */
function applyTimeCustomLabel(formatted: string, label: string): string {
  const body = formatted.trim();
  if (body === '0s' || body === '0') {
    return `0 ${label}`;
  }

  // Strip trailing time tokens (repeat for compound "1d 2h", "1d255ms").
  // Long names → multi-letter narrow (ms before m|s) → single-letter. One µ only (not µs|μs).
  let stripped = body.replace(/\u03bc/g, '\u00b5'); // Greek μ → micro sign µ (Intl variants)
  const timeToken =
    /(?:\s*)(?:milliseconds?|microseconds?|nanoseconds?|seconds?|minutes?|hours?|days?|weeks?|months?|years?|ms|us|ns|µs|[ywdhms])$/i;
  let guard = 0;
  while (timeToken.test(stripped) && guard++ < 8) {
    stripped = stripped.replace(timeToken, '').trimEnd();
  }
  if (stripped !== '' && stripped !== body) {
    // Prefer full numeric head (incl. decimals / compact) from stripped body
    const qty = leadingQuantity(stripped) ?? stripped;
    return `${qty} ${label}`;
  }
...
function applyCurrencyCustomLabel(formatted: string, label: string): string {
  ...
  // Keep optional leading sign so "-$1.23" → "-1.23 credits" (not "-$1.23 credits").
  let body = formatted.trim();
  body = body.replace(/^([+-]?)\s*[^\d\s+-]+/, '$1'); // -$1.23 / $1.23 / €12.3
