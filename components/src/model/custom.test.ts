...
describe('applyCustomLabel', () => {
  ...
  it('replaces Intl time narrow suffixes (ms, s)', () => {
    ...
    // Antoine: value=1 → never "1ms prout"
    expect(applyCustomLabel('1ms', 'prout', 'milliseconds')).toBe('1 prout');
    expect(applyCustomLabel('1.00ms', 'prout', 'milliseconds')).toBe('1.00 prout');
    expect(applyCustomLabel('1μs', 'prout', 'microseconds')).toBe('1 prout');
  ...
  it('currency: amount + custom label', () => {
    ...
    // Sign kept; currency symbol stripped (Copilot: -$1.23 → -1.23 credits)
    expect(applyCustomLabel('-$1.23', 'credits', 'usd')).toBe('-1.23 credits');
    expect(applyCustomLabel('$-1.23', 'credits', 'usd')).toBe('-1.23 credits');
  });
...
describe('formatValue with customLabel', () => {
  ...
  // Antoine: unit=milliseconds, value=1 → must be "1 prout", never "1ms prout"
  it('time milliseconds value=1: fully replaces glued ms', () => {
    const out = formatValue(1, { unit: 'milliseconds', customLabel: 'prout' });
    expect(out).toBe('1 prout');
    expect(out.toLowerCase()).not.toMatch(/ms/);
  });
  ...
  it('usd negative: no residual currency symbol', () => {
    const out = formatValue(-1.23, { unit: 'usd', customLabel: 'credits' });
    expect(out).toContain('credits');
    expect(out).toMatch(/-/);
    expect(out).not.toContain('$');
  });
